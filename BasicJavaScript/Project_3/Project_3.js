...
const setUsersToPremium = (users) => {
	...
	users.forEach(function(user) { user.isPremium = true; });
	return users;
};
...
const sumUserPostLikes = (user) => {
	...
	// return the sum
	return user.posts.reduce(function(total, current) { return total += current.likes; }, 0);
};
...
const addCalculateDiscountPriceMethod = (storeItem) => {
	...
	storeItem.calculateDiscountPrice = function() {
		store.discountPrice = storeItem.price - storeItem.price * storeItem.discountPercentage;
	};
	return storeItem;
};
